console.log('TauriUDPTransportService')
...
console.log('TauriUDPTransportService after import')


try {
  listen('your-event', (event) => {
    ...
    try {
      ...
      this.unlisten = await listen('plugin://udp', (event) => {
        ...
        if (payload.id === this.socketId) {
          const { remoteAddress, remotePort, buffer } = event.payload as {
            remoteAddress: string
            remotePort: number
            buffer: string | Uint8Array
          }
