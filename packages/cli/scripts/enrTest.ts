...
const enr = async () => {
  ...
  console.log('portal_historyAddEnr', add.result !== undefined ? 'pass' : 'fail')
  ...
  if (ping.result !== undefined) {
    ...
    console.log(
      'Enr Added: ',
      peers.result.buckets.flat().includes(nodeIds[1]) !== undefined ? 'pass' : 'fail',
    )
    // setTimeout(async () => {
    // }, 2000)
    const del = await ultralights[0].request('portal_historyDeleteEnr', [nodeIds[1]])
    console.log('portal_historyDeleteEnr', del.result !== undefined ? 'pass' : 'fail')
    setTimeout(async () => {}, 3000)
    const _peers = await ultralights[0].request('portal_historyRoutingTableInfo', [])
    console.log(
      'Enr Deleted: ',
      _peers.result.buckets.flat().includes(nodeIds[1]) !== undefined ? 'fail' : 'pass',
    )
