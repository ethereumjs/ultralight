...
const main = async () => {
    ...
    for (const update of updatesByRange.response!) {
        range.push(concatBytes(
            ...
            (ssz.allForksLightClient[update.version as LightClientForkName] as allForks.AllForksLightClientSSZTypes).LightClientUpdate.serialize(update.data),
        ))
    }
    ...
    for (let x = 0; x < 4; x++) {
        ...
        await ultralights[Math.floor(Math.random() * 10)].request('portal_beaconStore', [getBeaconContentKey(BeaconLightClientNetworkContentType.LightClientBootstrap, LightClientBootstrapKey.serialize({ blockHash: hexToBytes(bootstrapRoot) })), toHexString(concatBytes(beaconConfig.forkName2ForkDigest(bootstrap.version), (ssz.allForksLightClient[bootstrap.version as LightClientForkName] as allForks.AllForksLightClientSSZTypes).LightClientBootstrap.serialize(bootstrap.data)))])
    ...
    const res3 = await ultralights[0].request('portal_beaconStore', [optimisticUpdateKey, toHexString(concatBytes(beaconConfig.forkName2ForkDigest(optimisticUpdate.version), (ssz.allForksLightClient[optimisticUpdate.version as LightClientForkName] as allForks.AllForksLightClientSSZTypes).LightClientOptimisticUpdate.serialize(optimisticUpdate.data)))])
    ...
    while (true) {
        ...
        let optimisticUpdateKey = getBeaconContentKey(BeaconLightClientNetworkContentType.LightClientOptimisticUpdate, LightClientOptimisticUpdateKey.serialize({ optimisticSlot: BigInt(optimisticUpdate.data.attestedHeader.beacon.slot) }))  
        const res = await ultralights[0].request('portal_beaconStore', [optimisticUpdateKey, toHexString(concatBytes(beaconConfig.forkName2ForkDigest(optimisticUpdate.version), (ssz.allForksLightClient[optimisticUpdate.version as LightClientForkName] as allForks.AllForksLightClientSSZTypes) .LightClientOptimisticUpdate.serialize(optimisticUpdate.data)))])
