...
const remember = async (contentKey: Uint8Array, contentId: Uint8Array, content: Uint8Array) => {
  db.set(toHexString(contentKey), content)
}

const store = async (contentKey: Uint8Array, contentId: Uint8Array, content: Uint8Array) => {
...
const gossip = async (contentKey: Uint8Array, contentId: Uint8Array, content: Uint8Array) => {
  const client = jayson.Client.http({
...
const network = async (contentKey: Uint8Array, contentId: Uint8Array, content: Uint8Array) => {
  parentPort?.postMessage(
    `network/${toHexString(contentId)}/${toHexString(contentKey)}/${toHexString(content)}`,
  )
}
...
const toStorage = async (contentKey: Uint8Array, contentId: Uint8Array, content: Uint8Array) => {
  const memory: 'network' | 'store' | 'gossip' | 'remember' = workerData.memory ?? 'remember'
  const memoryFn = to_storage[memory]
  await memoryFn(contentKey, contentId, content)
}
...
const generateStateNetworkContent = async () => {
  ...
  for (const accessList of accessLists) {
    for (const contract of accessList) {
      const { storageProof, address, accountProof, balance, codeHash, nonce, storageHash } =
        await alchemy.core.send('eth_getProof', [contract.address, contract.storageKeys, number])
      const accountProofContent = AccountTrieProofType.serialize({
        witnesses: accountProof.map(fromHexString),
        nonce: BigInt(nonce),
        balance: BigInt(balance),
        storageRoot: fromHexString(storageHash),
      ...
      const accountProofContentId = getStateNetworkContentId({
        contentType: StateNetworkContentType.AccountTrieProof,
        address: Address.fromString(address),
      })

      await toStorage(accountProofContentKey, accountProofContentId, accountProofContent)
      ...
      const bytecodeContentId = getStateNetworkContentId({
        address: Address.fromString(address),
        contentType: StateNetworkContentType.ContractByteCode,
        codeHash: fromHexString(codeHash),
      })
      const contractBytecode = ContractByteCodeType.serialize(fromHexString(bytecode))
      await toStorage(bytecodeContentkey, bytecodeContentId, contractBytecode)
      for (const p of storageProof) {
        const contentkey = getStateNetworkContentKey({
          ...
          address: Address.fromString(contract.address),
          slot: BigInt(p.key),
          stateRoot: fromHexString(stateroot),
        })
        ...
        totalCSP++
        await toStorage(contentkey, contentId, csp)
      }
    }
  }
  ...
  if (contracts.length > 0) {
    for (const c of contracts) {
      ...
      const bytecode = await alchemy.core.getCode(c.contractAddress)
      const bytecodeContentkey = getStateNetworkContentKey({
        address: Address.fromString(c.contractAddress),
        contentType: StateNetworkContentType.ContractByteCode,
        codeHash: fromHexString(codeHash),
  ...
  for (const add of Object.keys(accountsData)) {
    ...
    await toStorage(contentKey, contentId, content)
  }
