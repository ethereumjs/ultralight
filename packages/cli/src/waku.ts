  ...
  implements ITransportService
{
  ...
  deleteObserver() {
    ...
    this.log('subscribing to: ' + this.contentTopic + this.nodeId)
  ...
  async processIncomingMessage(wakuMessage: DecodedMessage) {
    ...
    if (m.userId !== undefined) {
      // this.log(`Processing incoming Message: from ${m.userId}`)
      this.emit('rtcMessage', m.userId, message.text)
    } else {
      m.address && (await this.handleIncoming(payload))
      // this.log(`Processing incoming Message: from ${m.address.toString()}`)
    }
    // }
  }

  async send(to: any, toId: string, packet: IPacket) {
    // this.log('Sending message to: ' + to.toString())
    const message = {
      address: to.toString(),
  ...
  public async handleIncoming(data: Uint8Array) {
    ...
    try {
      ...
      this.emit('packet', multi as any, packet)
    } catch (e) {
      this.emit('decodeError', e as Error, multi as any)
    }
  }

  async sendMessage(message: string, toId: string = ''): Promise<SendResult | undefined> {
    // this.log('Sending message to: ' + toId)
