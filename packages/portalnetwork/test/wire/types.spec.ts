...
tape('message encoding should match test vectors', (t) => {
    ...
    let testVector = "0001000000000000000c000000feffffffffffffffffffffffffffffffffffffffffffffffffffffffffffffff"
    ...
    payload = PortalWireMessageType.serialize({ selector: MessageCodes.FINDNODES, value: { distances } })
    ...
    payload = PortalWireMessageType.serialize({
        selector: MessageCodes.NODES, value: {
            total: 1,
            enrs: [],
        }
    })
    testVector = "030105000000"
    t.isEqual(Buffer.from(payload).toString('hex'), testVector, 'nodes message with no ENRs encoded correctly')

    // Validate NODES message encoding
    const enr1 = "enr:-HW4QBzimRxkmT18hMKaAL3IcZF1UcfTMPyi3Q1pxwZZbcZVRI8DC5infUAB_UauARLOJtYTxaagKoGmIjzQxO2qUygBgmlkgnY0iXNlY3AyNTZrMaEDymNMrg1JrLQB2KTGtv6MVbcNEVv0AHacwUAPMljNMTg"
    const enr2 = "enr:-HW4QNfxw543Ypf4HXKXdYxkyzfcxcO-6p9X986WldfVpnVTQX1xlTnWrktEWUbeTZnmgOuAY_KUhbVV1Ft98WoYUBMBgmlkgnY0iXNlY3AyNTZrMaEDDiy3QkHAxPyOgWbxp5oF1bDdlYE6dLCUUp8xfVw50jU"
    const total = 1
    ...
    testVector = ""
    t.isEqual(Buffer.from(payload).toString('hex'), testVector, 'nodes message with 2 ENRs encoded correctly')


    // Validate FINDCONTENT message encoding
    const contentKey = fromHexString("0x706f7274616c")
    payload = PortalWireMessageType.serialize({ selector: MessageCodes.FINDCONTENT, value: { contentKey: contentKey } })
    testVector = "0404000000706f7274616c"
    t.isEqual(Buffer.from(payload).toString('hex'), testVector, 'find content message encoded correctly')
    ...
    payload = PortalWireMessageType.serialize({ selector: MessageCodes.OFFER, value: { contentKeys: contentKeys } })
