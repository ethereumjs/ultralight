...
tape('history Protocol FINDCONTENT/FOUDNCONTENT message handlers', async (t) => {
  ...
  await protocol.store(
    fromHexString(getContentKey(ContentType.BlockHeader, fromHexString(block1Hash))),
...
tape('store -- Headers and Epoch Accumulators', async (t) => {
    ...
    await protocol.store(
      fromHexString(
        getContentKey(
          ...
          fromHexString('0x5ec1ffb8c3b146f42606c74ced973dc16ec5a107c0345858c343fc94780b4218')
        )
      ),
      fromHexString(epoch)
    )
  ...
  t.test('Should store and retrieve an EpochAccumulator from DB', async (st) => {
    ...
    await protocol.store(
      fromHexString(getContentKey(ContentType.EpochAccumulator, hashRoot)),
...
tape('store -- Block Bodies and Receipts', async (t) => {
  ...
  await protocol.store(
    fromHexString(getContentKey(ContentType.EpochAccumulator, fromHexString(epochHash))),
    fromHexString(epoch)
  )
  ...
  protocol.store(
    fromHexString(getContentKey(ContentType.BlockHeader, fromHexString(serializedBlock.blockHash))),
    headerWithProof
  )
  await protocol.store(
    fromHexString(getContentKey(ContentType.BlockBody, fromHexString(serializedBlock.blockHash))),
...
tape('Header Proof Tests', async (t) => {
  ...
  t.test(
    ...
    async (st) => {
      ...
      await protocol.store(
        fromHexString(getContentKey(ContentType.EpochAccumulator, fromHexString(_epoch1Hash))),
      ...
      await protocol.store(
        fromHexString(getContentKey(ContentType.BlockHeader, fromHexString(_block1000.hash))),
        headerWith
      )
