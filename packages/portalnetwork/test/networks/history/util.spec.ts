import { createBlockFromRLP, createBlockFromRPC, createBlockHeader } from '@ethereumjs/block'
...
describe('BlockBody ssz serialization/deserialization', () => {
  ...
  const block = createBlockFromRLP(hexToBytes(block200031Rlp), {
    setHardfork: true,
  })
...
describe('BlockHeader ssz serialization/deserialization with pre and post shanghai blocks', () => {
  it('should serialize and deserialize pre and post shanghai blocks', async () => {
    ...
    // @ts-expect-error wants 0xstring
    const preShanghaiBlock = createBlockFromRPC(preShanghaiBlockJson, undefined, {
      setHardfork: true,
    })
    // @ts-expect-error wants 0xstring
    const postShanghaiBlock = createBlockFromRPC(postShanghaiBlockJson, undefined, {
      setHardfork: true,
    })
