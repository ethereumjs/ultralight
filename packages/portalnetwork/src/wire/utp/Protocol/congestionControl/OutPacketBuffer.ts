...
export default class OutPacketBuffer {
  ...
  private buffer: UtpPacketDTO[] = new Array(OutPacketBuffer.size)
   ...
   */
  public markPacketAcked(
    seqNrToAck: number,
    timestamp: number,
    ackSmallerThanThisSeq: boolean
  ): number {
    let bytesJustAcked: number = -1
    const pkt: UtpPacketDTO | null = this.findPacket(seqNrToAck)
    if (pkt != null) {
      if ((pkt.utpPacket.header.seqNr & 0xffff) == seqNrToAck) {
        if (!pkt.isPacketAcked) {
          const payloadLength: number =
            pkt.utpPacket.payload == null ? 0 : pkt.utpPacket.payload.length
          bytesJustAcked = payloadLength + DEF_HEADER_LENGTH
        }
  ...
  private findPacket(seqNrToAck: number): UtpPacketDTO | null {
    if (this.buffer.length > 0) {
      const firstSeqNr: number = this.buffer[0].utpPacket.header.seqNr & 0xffff
      ...
      if (
        ...
        for (let i = 0; i < this.buffer.length; i++) {
          const pkt: UtpPacketDTO = this.buffer[i]
          if ((pkt.utpPacket.header.seqNr & 0xffff) == seqNrToAck) {
            return pkt
          }
        }
  ...
  public removeAcked(): void {
    const toRemove = new Array(OutPacketBuffer.size)
  ...
  private resendRequired(unackedPkt: UtpPacketDTO): boolean {
    ...
    const timedOut: boolean = this.isTimedOut(unackedPkt)
  ...
  private isTimedOut(utpPacketDTO: UtpPacketDTO): boolean {
    const delta: number = this.currentTime - utpPacketDTO.stamp
