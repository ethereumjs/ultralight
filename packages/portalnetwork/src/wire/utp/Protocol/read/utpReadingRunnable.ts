...
export class utpReadingRunnable {
  ...
  constructor(
    ...
    this.lastPacketTimestamp = synAck.header.timestamp;
  ...
  timeAwaitedAfterLastPacket() {
    return (
      MicrosecondTimeStamp() - this.lastPacketTimestamp > 2 &&
      this.gotLastPacket
    );
  }
    ...
    while (this.continueReading()) {
      ...
      try {
        ...
        if (
          ...
          err instanceof ArrayIndexOutOfBoundsException
        )
          this.exp = err;
  ...
  isTimedOut() {
    let timedOut =
      Number(this.nowTimestamp) - this.lastPacketReceived >= 4000000;
    let connectionReattemptAwaited =
      Number(this.nowTimestamp) - this.startReadingTimeStamp >= 4000000;
  ...
  handleExpectedPacket(Pair: UtpPacketDTO) {
    if (this.hasSkippedPackets()) {
      ...
      packets.forEach((packet) => {
        this.buffer?.write(
          packet.utpPacket.payload.toString(),
          this.buffer.length
        );
      ...
      if (this.hasSkippedPackets()) {
        if (this.ackThisPacket()) {
          ...
          this.socket.sendSelectiveAck(
            headerExtension,
            this.getTimeStampDifference(Pair),
            this.getLeftSpaceInBuffer()
          );
        }
      } else {
        if (this.ackThisPacket()) {
        ...
        this.buffer?.write(
          Pair.utpPacket.payload.toString(),
          this.buffer.length
        );
  ...
  handleUnexpectedPacket(Pair: UtpPacketDTO) {
    ...
    let alreadyAcked =
      expected > seqNr || seqNr - expected > this.PACKET_DIFF_WARP;
    ...
    if (sameSeqNr && !alreadyAcked) {
      ...
      if (this.ackThisPacket()) {
        this.socket.sendSelectiveAckPacket(
          headerExtension,
          this.getTimeStampDifference(Pair),
          this.getLeftSpaceInBuffer()
        );
