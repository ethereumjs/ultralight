...
const packetComparator: Comparator<Packet<PacketType>> = (
  a: Packet<PacketType>,
  b: Packet<PacketType>,
) => {
  ...
  if (a.header.connectionId === b.header.connectionId) {
    return a.header.pType === PacketType.ST_STATE
      ? a.header.ackNr - b.header.ackNr
      : a.header.seqNr - b.header.seqNr
  }
...
export class RequestManager {
  ...
  lookupRequest(connectionId: number): ContentRequest | 'closed' | undefined {
    const request =
      ...
      this.requestMap[connectionId - 1] ??
      this.requestMap[connectionId + 1]
    return request
  }
  ...
  getPacketCount(connectionId: number): number {
    return this.packetHeap.heapArray.filter((packet) => packet.header.connectionId === connectionId)
      .length
  }
  ...
  async processCurrentPacket(): Promise<void> {
    ...
    this.logger.extend('PROCESS_CURRENT_PACKET')(
      `Processing ${PacketType[this.currentPacket.header.pType]} [${this.currentPacket.header.pType === PacketType.ST_STATE ? this.currentPacket.header.ackNr : this.currentPacket.header.seqNr}] for Req:${this.currentPacket.header.connectionId}`,
    )
    const request = this.lookupRequest(this.currentPacket.header.connectionId)
    if (request === undefined || request === 'closed') {
      this.logger.extend('PROCESS_CURRENT_PACKET')(
        `Request: ${request} for current packet - connectionId: ${this.currentPacket.header.connectionId}.  Skipping packet.`,
      )
      this.currentPacket = this.packetHeap.pop()
      await this.processCurrentPacket()
      return
    ...
    if (request.socket.type === UtpSocketType.READ && request.socket.reader !== undefined) {
      if (this.currentPacket.header.seqNr < request.socket.reader.nextDataNr) {
        ...
        if (this.getPacketCount(this.currentPacket.header.connectionId) < MAX_IN_FLIGHT_PACKETS) {
          ...
          this.logger.extend('PROCESS_CURRENT_PACKET')(
            `Packet is ahead of current reader position - seqNr: ${this.currentPacket.header.seqNr} > ${request.socket.reader?.nextDataNr}.  Pushing packet back to heap.`,
          )
          ...
          this.logger.extend('PROCESS_CURRENT_PACKET')(
            `Packet is ahead of current reader position - seqNr: ${this.currentPacket.header.seqNr} > ${request.socket.reader?.nextDataNr}.  Treating expected packet as lost.`,
          )
        }
      }
    }
