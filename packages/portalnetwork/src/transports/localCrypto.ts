...
// Improved AES-CTR implementation
class AesCtr {
  ...
  constructor(key: Uint8Array, iv: Uint8Array) {
    ...
    // Create working copy of counter
...
// More robust browser crypto implementation
class BrowserCrypto {
  ...
  static async createDecipheriv(algorithm: string, key: Uint8Array, iv: Uint8Array): Promise<any> {
    console.log('Creating decipher with algorithm:', algorithm);
    console.log('Key (first 4 bytes):', Array.from(key.slice(0, 4)));
    console.log('IV (first 4 bytes):', Array.from(iv.slice(0, 4)));
